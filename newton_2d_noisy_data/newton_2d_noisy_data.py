...
for i in range(1, 51):
    ...
    val_x_noisy = val_x_ + (random.random() - 0.5)*0.2*val_x_
    val_y_noisy = val_y_ + (random.random() - 0.5)*0.2*val_y_
    val_x_v_noisy = val_x_v_ + (random.random() - 0.5)*0.2*val_x_v_
    val_y_v_noisy = val_y_v_ + (random.random() - 0.5)*0.2*val_y_v_
    answers.append([val_x_noisy, val_y_noisy, val_x_v_noisy, val_y_v_noisy, dt*i])

answers_np = np.array(answers)
plt.scatter(answers_np[:, 4], answers_np[:, 0], label="x")
plt.scatter(answers_np[:, 4], answers_np[:, 1], label="y")
plt.scatter(answers_np[:, 4], answers_np[:, 2], label="u")
plt.scatter(answers_np[:, 4], answers_np[:, 3], label="v")
plt.legend()
plt.show()
...
plt.scatter(dataset_with_noise[:, 4], dataset_with_noise[:, 0], label="x")
plt.scatter(dataset_with_noise[:, 4], dataset_with_noise[:, 1], label="y")
plt.scatter(dataset_with_noise[:, 4], dataset_with_noise[:, 2], label="u")
plt.scatter(dataset_with_noise[:, 4], dataset_with_noise[:, 3], label="v")
